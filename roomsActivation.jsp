...
<link rel="STYLESHEET" type="text/css" href="Xtyle.css">
	...
	<tr class="<%= tr_class %>">
		...
		<td align="center">
		<% if (room.active) { %>
		<a href="deactivateRoomContract.jsp?room_id=<%= room.id %>&contract_id=<%= contract_id %>">deactivate</a>
		<% } else { %>
		<a href="activateRoomContract.jsp?room_id=<%= room.id %>&contract_id=<%= contract_id %>">activate</a>
		<% } %>
		</td>
	</tr>
